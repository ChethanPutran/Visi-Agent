...
from src.services.api_gateway.app.schemas.video_schemas import VideoListResponse, VideoStatus
from src.services.video_ingestion.app.contracts.schemas import VideoUploadResponse, BatchUploadResponse
from src.services.video_processing.app.contracts.schemas import ProcessingStage, ProcessingStageInfo, VideoProcessingStatus
...
async def upload_video(
    ...
    try:
        logger.log(logging.DEBUG,
                   f"Request to endpoint video: {file.filename}")
        # Validate file
        ...
        logger.log(
            logging.DEBUG, f"Video uploaded successfully: {metadata.id} - {metadata.filename}")
        logger.log(logging.DEBUG, f"Scheduling processing for video: {metadata.id} - {metadata.filename}") if prcess_immediately else logger.log(
            logging.DEBUG, f"Processing not scheduled for video: {metadata.id} - {metadata.filename}")
...
async def get_video_status(
    ...
    try:
        ...
        logger.log(
            logging.DEBUG, f"Status for video {video_id}: {status.current_stage if status else 'Not found'}")
        if not status:
            metadata = await video_service.get_video_metadata(video_id)
            logger.log(
                logging.DEBUG, f"Metadata for video {video_id}: {metadata.id if metadata else 'Not found'}")
            if not metadata:
                raise HTTPException(status_code=404, detail="Video not found")
            if metadata.process_end_time is None:
                return VideoProcessingStatus(
                    video_id=video_id,
                    current_stage=ProcessingStage.NOT_STARTED,
                    progress=0.0,
                    message="Video uploaded but not processed yet",
            ...
            return VideoProcessingStatus(
                ...
                error=None,
                estimated_completion=metadata.process_end_time
            )
        return status
...
async def get_video_metadata(
    ...
    """
    logger.log(logging.DEBUG,
               f"Request to endpoint video/metadata: {video_id}")
    try:
        metadata = await video_service.get_video_metadata(video_id)
...
async def process_video(
    ...
    """
    logger.log(logging.DEBUG, f"Request to endpoint video/process")

    # Get video metadata
    metadata = await video_service.get_video_metadata(video_id)
    logger.log(logging.DEBUG, f"Video metadata for processing: {metadata.id}") if metadata else logger.log(
        logging.DEBUG, f"No metadata found for video: {video_id}")
...
async def get_video_transcript(
    ...
    try:
        ...
        transcript_text = transcript if isinstance(transcript, str) else transcript.get(
            "text", "") if isinstance(transcript, dict) else ""
...
async def upload_videos_batch(
    ...
    try:
        ...
        logger.log(logging.DEBUG,
                   f"Uploading videos... Batch size: {len(files)}")
